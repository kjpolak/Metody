...
(define (laz-fib m n)
  (lcons m
         (lambda () (laz-fib n (+ m n)))))

(define lazy-fibonacci
  (laz-fib 0 1))

(define (integer-from m)
  (lcons m
        (if (>= m 0)
            (lambda () (integer-from (- (+ m 1))))
            (lambda () (integer-from (- m))))))
...
(define (xappend xs ys)
  (if (null?? xs)
      ys
      (cons-cons (car-expr xs) (xappend (cdr-expr xs) ys))))
